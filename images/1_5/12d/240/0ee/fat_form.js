...
function Form_ScrollToErrorMessage(iFormNodeID){var error_msg=$('form_invalid_msg_holder_'+iFormNodeID);if(!Form_IsElementVisible(error_msg)){$(!Browser.Engine.webkit?document.documentElement:document.body).scrollTop=error_msg.getPosition(document).y;}}
function Form_IsElementVisible(element){var topscroll=$(!Browser.Engine.webkit?document.documentElement:document.body).scrollTop;var win_height=$(window).getHeight();var elem_top=element.getPosition().y;var elem_height=element.getHeight();if(topscroll<=elem_top&&topscroll+win_height>=elem_top+elem_height){return true;}
return false;}
...
function Form_SetInvalidFieldDecoration(oField){oField=$(oField);var sErrMsgHolderId=Form_GetFieldErrHolderId(oField);var oErrMsgHolder=document.getElementById(sErrMsgHolderId);if(oErrMsgHolder==null){var sValidatorMessage='';if(oField.getAttribute('type')=='checkbox'){sValidatorMessage='This is a mandatory field.';}else if(oField.getAttribute('type')=='radio'){sValidatorMessage='Please select one of the radio button options.';}else{sValidatorMessage=Validator_GetMessage(oField);}
...
function Form_SetValidFieldDecoration(oField){$(oField).removeClass('form-field-error');var sErrMsgHolderId=Form_GetFieldErrHolderId(oField);var oErrMsgHolder=document.getElementById(sErrMsgHolderId);if(oErrMsgHolder!=null){oErrMsgHolder.parentNode.removeChild(oErrMsgHolder);}}
...
return null;}
function Field_HandleChange(){Form_SetValidFieldDecoration(this);$(this).removeEvents({keyup:arguments.callee,change:arguments.callee});}
...
Form_RegisterTooltip(oField,sHint);}
...
return false;}
$(window).addEvent('domready',function(){for(var i=0,l=Form_RegisteredTooltips.length;i<l;i+=1){var field=Form_RegisteredTooltips[i].field;var hint=Form_RegisteredTooltips[i].hint;if(!field.getWidth()){Form_ShowParentContainers(field);}
var tip=new SK.UI.Tip($(field),'',hint,{onoffset:function(e){return{x:e.getSize().x,y:0};}});}
...
function Validator_RegExp(vValue,oOptions){if(isDefined(oOptions)&&isDefined(oOptions.reg_exp)){var sRegExp=oOptions.reg_exp;if(sRegExp.indexOf('/')==-1){sRegExp='/'+sRegExp+'../0101.nccdn.net/default.htm';}
